package com.sparkdata.utils

import java.io.FileNotFoundException
import java.net.URI
import com.sparkdata.SparkDistCPOptions
import com.sparkdata.objects.CopyActionResult.SkippedAlreadyExists
import com.sparkdata.objects.DeleteActionResult.{Deleted, SkippedDoesNotExists, SkippedDryRun}
import com.sparkdata.objects.{CopyActionResult, DeleteActionResult, DeleteResult, DirectoryCopyResult, DistCPResult, FileCopyResult, Logging, SerializableFileStatus, SingleCopyDefinition}
...
object CopyUtils extends Logging {
  ...
  def handleCopy(sourceFS: FileSystem, destFS: FileSystem, definition: SingleCopyDefinition, options: SparkDistCPOptions, taskAttemptID: Long): DistCPResult = {
    ...
    logInfo(s"handleCopy source:${definition.source.uri} dest:${definition.destination}")
    val r = {
      if (definition.source.isDirectory) {
        logInfo(s"definition.source.isDirectory")
        CopyUtils.createDirectory(destFS, definition, options)
      }
      else if (definition.source.isFile) {
        logInfo(s"definition.source.isFile")
  ...
  private[utils] def deleteFile(fs: FileSystem, path: Path, options: SparkDistCPOptions): DeleteResult = {
    if (!fs.exists(path)) {
      ...
      DeleteResult(path.toUri, SkippedDryRun)
    }
    else {
      Try(fs.delete(path, true)) match {
        case Success(true) => DeleteResult(path.toUri, Deleted)
  ...
  private[utils] def createDirectory(destFS: FileSystem, definition: SingleCopyDefinition, options: SparkDistCPOptions): DirectoryCopyResult = {
    ...
    if (destFS.exists(destPath)) {
      DirectoryCopyResult(definition.source.getPath.toUri, definition.destination, SkippedAlreadyExists)
    }
    else if (options.dryRun) {
